; (function($) {
    ...
    var Item = function(element, options) {
        ...
        if (options.mirror && this.element.tagName === 'div') {
        ...
        this.moveTo = function(x, y, scale) {
            ...
            if (transform && options.transforms) {
                ...
                if (options.mirror && this.element.tagName === 'div') this.reflection.style.marginTop = (options.mirror.gap * scale) + "px";
                style.width = this.width + "px";
                style.left = x + "px";
                style.top = y + "px";
            }
			if(scale >= 0.95) {
				style.background = "linear-gradient(to right,rgba(255,191,19,0.5),rgba(255,213,45,0.05))";
				style.borderLeft = "2px solid #ffcc2f";
				style.color = "#ffffff";
				style.fontWeight = "bold"
			}else{
				style.background = "linear-gradient(to right,rgba(28,250,254,0.3),rgba(66,252,255,0.05))";
				style.borderLeft = "2px solid #53fdff";
				style.color = "#c7fffe";
    ...
    var requestFrame = window.requestAnimationFrame; (function() {
        var vendors = ['webkit', 'moz', 'ms'];
        for (var i = 0,
    ...
    var Carousel = function(element, options) {
        ...
        this.enableAutoPlay = function() {
            $(".yearContent").bind('mouseover.cloud9',
            function() {
                clearInterval(self.autoPlayTimer);
                debugger
                clearInterval(window.timer);
            });
            $(".yearContent").bind('mouseout.cloud9',
            function() {
                self.autoPlay();
                window.autoPlay();
            });
        ...
        this.bindControls = function() {
			if (options.stopBtn) {
				options.stopBtn.bind('click',
				function() {
					var active=$('.sys-stop').hasClass('active');
					console.log(active)
					if(!active){
						self.autoPlay();
						// $(".yearContent").bind('click');
						$(".yearContent").bind('click.cloud9',
							function(event) {
						...
						clearInterval(self.autoPlayTimer);	
						//$(".yearContent").unbind('click');
						$(".yearContent").unbind('click.cloud9');
					}
				    return false;
				});
			}
            ...
            if (options.mouseWheel) {
                $(".yearContent").bind('mousewheel.cloud9',
        ...
        this.finishInit = function() {
            for (var i = 0; i < items.length; i++) {
                var item = items[i];
                if ((item.tagName === 'div') && ((item.width === undefined) || ((item.complete !== undefined) && !item.complete))) return;
            }
